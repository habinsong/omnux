...
- **디자인 빅뱅 재구축 (2026-06-04, UIUX_design.md 준수)**: 4천 줄 레거시 모놀리식 `App.css`를 전면 폐기(→303줄)하고 **Tailwind CSS v4 + shadcn/ui 토큰 아키텍처**로 데스크톱 프론트를 빅뱅 재구축했다. `@tailwindcss/vite` 도입, `App.css`를 Tailwind 엔트리(시맨틱 토큰 + `@theme inline` + base + `.prose-omnux`)로 전환. **3-tier 테마**(🪟Glass 기본=파스텔 blob+backdrop-blur, 🌞Light=오프화이트+그림자, 🌙Dark=warm `#111113`/`#1A1A1E`+인디고 glow) + Primary=indigo. 셸(App/Nav/TopBar 3-모드 토글)·10개 페이지·셸 서브컴포넌트·다이얼로그·에러 경계 전부 Tailwind로 재작성하고 레거시 클래스를 제거했다. 공용 프리미티브 `src/components/ui/primitives.tsx`(Button/Card/Badge/Input/Textarea/EmptyState/Spinner 등) 신설. `check-desktop-shell-boundary-contract`(682 assertions)·`apps/desktop npm run build`·repo-hygiene·`git diff --check` green, 3 테마 브라우저 프리뷰 렌더 확인.
- **백엔드 hidden feature 연결 착수 — LLM 설정**: backend_hidden_features.md 미연결분 중 LLM provider/model 설정을 데스크톱에 1차 연결했다. `desktop-message-gateway`에 `requestDesktopLlm`(get/set_groq_model·copilot_model, get_copilot/codex_status, get_usage_stats, set/delete_llm_credentials, start_copilot_login) 추가, `settings-store`에 모델/상태/사용량 처리, `LlmModelsPanel.tsx`로 Groq/Copilot 모델 선택·적용·새로고침, CLI 어댑터 상태, API 키 저장/삭제, Gemini 토큰·Copilot Premium 쿼터 표시 UI를 Settings → Models 탭에 추가. 나머지 hidden feature(Think+/web 토글·Notebook/Handoff·STT·Gist 동기화·코딩 프리뷰 iframe·스킬 엔진 등)는 후속 연결 대상.
