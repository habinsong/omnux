...
명시 target이 필요하면 다음처럼 보낼 수 있다. upstream이 이미 있으면 기존 upstream과 다른 target은 백엔드가 차단한다.
...
- `push_current_branch`는 현재 브랜치 `HEAD`만 push한다. force push는 없다.
- `push_current_branch`는 `main`, `master`, `develop`, `dev`, `trunk`, `release/*`, `hotfix/*`를 차단한다.
...
```json
{
  ...
  "payload": {
    ...
    "offlineMode": {
      "requested": true,
      "status": "ready_for_manual_routing",
      "requestedBy": ["OMNUX_OFFLINE_MODE"],
      "cloudProviderKeysPresent": ["OMNUX_GROQ_API_KEY_FILE"],
      "checks": [
        {
          "name": "offline_flag",
          "status": "ok",
          "message": "offline mode was requested"
        },
        {
          "name": "local_models",
          "status": "ok",
          "message": "at least one local model is available"
        },
        {
          "name": "cloud_credentials",
          "status": "warning",
          "message": "cloud provider key env vars are configured"
        },
        {
          "name": "provider_routing",
          "status": "skipped",
          "message": "LocalLlmProvider routing is not enabled yet"
        },
        {
          "name": "traffic_guard",
          "status": "skipped",
          "message": "external HTTP traffic blocking is not enabled yet"
        }
      ]
    },
    "warnings": [],
...
- `offlineMode.status`는 `not_requested`, `ready_for_manual_routing`, `blocked` 중 하나다.
- `offlineMode.requestedBy`는 truthy로 설정된 `OMNUX_OFFLINE_MODE`, `OMNUX_LOCAL_LLM_ONLY`, `OMNUX_DISABLE_CLOUD_LLM` 중 env var 이름이다.
- `offlineMode.cloudProviderKeysPresent`는 설정된 클라우드 key env var 이름만 포함하며 값은 포함하지 않는다.
