...
async (page) => {
  ...
  if(await composer.getByRole('combobox',{name:'작업 방식',exact:true}).isVisible())throw Error('작업 설정이 기본으로 펼쳐져 있습니다.');
  await composer.getByRole('textbox',{name:'만들거나 바꾸고 싶은 내용',exact:true}).fill('파일을 만들고 동작을 확인해 주세요.');
  const settings=composer.locator('summary').filter({hasText:'모델과 작업 설정'});await settings.focus();await settings.press('Enter');
  await composer.getByRole('button',{name:'모델 목록 새로고침',exact:true}).click();
  await composer.getByRole('combobox',{name:'담당 모델 제공자',exact:true}).selectOption('grok');
  await composer.getByRole('combobox',{name:'담당 모델',exact:true}).selectOption('fixture-custom-model');
  await composer.getByRole('combobox',{name:'작업 방식',exact:true}).selectOption('orchestration');
  await composer.getByRole('button',{name:'만들기',exact:true}).click();await root.getByRole('alert').filter({hasText:'한 개 이상'}).waitFor();
  if(requests.some(request=>request.type.startsWith('coding_run_')))throw Error('보조 모델 없이 요청했습니다.');
  await composer.getByRole('combobox',{name:'Grok 보조 모델',exact:true}).selectOption('fixture-custom-model');
  await composer.getByRole('textbox',{name:'빌드 이름',exact:true}).fill('나의 첫 빌드');
  await composer.getByRole('textbox',{name:'프로젝트 이름',exact:true}).fill('새 프로젝트');
  ...
  const references=composer.locator('summary').filter({hasText:'스킬과 참고 노트'});await references.click();
  await composer.getByRole('combobox',{name:'적용할 스킬',exact:true}).selectOption('project:code-review');await composer.getByRole('checkbox',{name:'작업 규칙.md',exact:true}).check();await references.click();
  ...
  await result.locator('summary').filter({hasText:'모델별 결과 비교'}).click();
  ...
  await settings.click();await composer.getByRole('textbox',{name:'빌드 이름',exact:true}).fill('바꾼 빌드 이름');await composer.getByRole('button',{name:'이름·프로젝트 저장',exact:true}).click();
  await root.getByRole('alert').filter({hasText:'모의 이름 저장 오류'}).waitFor();if(await page.getByText('모의 이름 저장 오류',{exact:true}).count()!==1)throw Error('화면 오류와 전역 알림을 중복으로 표시했습니다.');if(await composer.getByRole('textbox',{name:'빌드 이름',exact:true}).inputValue()!=='바꾼 빌드 이름')throw Error('저장 오류로 이름을 잃었습니다.');
  await layout('error',320);await composer.getByRole('button',{name:'이름·프로젝트 저장',exact:true}).click();await result.getByRole('heading',{name:'바꾼 빌드 이름',exact:true}).waitFor();await settings.click();
  ...
  await settings.click();await composer.getByRole('combobox',{name:'작업 방식',exact:true}).selectOption('single');await settings.click();
  ...
  await root.getByRole('button',{name:'새 빌드',exact:true}).click();
  const history=root.locator('summary').filter({hasText:/^저장한 빌드$/});await history.click();
  ...
  await settings.click();await composer.getByRole('combobox',{name:'작업 방식',exact:true}).selectOption('multi');await settings.click();
