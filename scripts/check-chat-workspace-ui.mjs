...
async (page) => {
  ...
  if(await root.getByRole('combobox',{name:'모드',exact:true}).isVisible())throw Error('설정이 기본으로 펼쳐져 있습니다.');
  const options=page.getByRole('tab',{name:'모델',exact:true});
  await options.click();await root.getByRole('combobox',{name:'모드',exact:true}).waitFor();
  ...
  await page.getByRole('tab',{name:'대화',exact:true}).click();await input.fill('첨부와 한글 조합을 확인해 주세요.');
  ...
  const resources=page.getByRole('tab',{name:'참고 자료',exact:true});await resources.click();
  ...
  await page.getByRole('tab',{name:'대화',exact:true}).click();await root.getByRole('button',{name:'질문 보내기',exact:true}).click();
  ...
  await history.click();await options.click();await root.getByRole('combobox',{name:'모드',exact:true}).selectOption('multi');
  for(const provider of ['Groq','Gemini','Cerebras','NVIDIA NIM','Copilot','Codex','Grok'])await root.getByRole('combobox',{name:`${provider} 워커 모델`,exact:true}).selectOption('none');
  await root.getByRole('combobox',{name:'Grok 워커 모델',exact:true}).selectOption('grok-fixture-custom');await root.getByRole('combobox',{name:'Codex 워커 모델',exact:true}).selectOption('fixture-codex');
  ...
  await options.click();holdChat=false;await page.getByRole('tab',{name:'대화',exact:true}).click();await input.fill('모델별 관점을 비교해 주세요.');await root.getByRole('button',{name:'질문 보내기',exact:true}).click();
  await page.waitForFunction(()=>!!window.__chatStore.getState().multiResult&&!window.__chatStore.getState().pending);
  await root.locator('summary').filter({hasText:/^멀티 결과$/}).click();
  ...
  await page.evaluate(()=>window.__chatNavigation.getState().setActivePage('ask',{input:'다른 화면의 요청'}));await root.getByText('다른 화면에서 전달받은 요청이 있습니다.').waitFor();if(await input.inputValue()!==failing.text)throw Error('전달받은 요청이 기존 초안을 덮어썼습니다.');
  await input.fill('');await root.getByRole('button',{name:'가져오기',exact:true}).click();if(await input.inputValue()!=='다른 화면의 요청')throw Error('전달된 초안을 가져오지 못했습니다.');
  ...
  await history.click();await options.click();await root.getByRole('combobox',{name:'모드',exact:true}).selectOption('orchestration');await page.getByRole('tab',{name:'대화',exact:true}).click();
