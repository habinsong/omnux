...
| 자동 커밋/PR 생성 | ✅ 실행 게이트 3차 | `GitAutomationSnapshotService`, `GitOperationPreviewService`, `git_automation_snapshot_get`, `git_operation_preview/apply` — branch/commit/push/PR 생성을 승인 게이트로 실행 |
...
- ✅ 실행 게이트 3차: `open_pull_request`를 추가했다. preview에서 `gh --version`, `gh auth status`, upstream sync, base/title 정책을 확인하고 apply에서 `gh pr create`를 실행한다.
...
- ✅ `open_pull_request`는 protected branch, detached HEAD, unpushed commit, behind 상태, base/title 누락, `gh` CLI/auth 실패를 차단한다.
- ⏳ 보류: rollback 실행, worktree remove/cleanup, `CodingApplicationService` 완료 훅 자동 호출, LLM 커밋 메시지/PR 본문 생성.
...
- **구현 방향**: 조회는 `GitAutomationSnapshotService`에 남기고, 실행은 `preview → apply` 승인 게이트로 분리한다. 현재 실행은 branch/commit/snapshot commit/current branch push/PR 생성까지 허용하며, rollback/worktree cleanup은 별도 정책 확정 후 연결한다.
...
- `LocalLlmOfflineModePolicy`가 `OMNUX_OFFLINE_MODE`, `OMNUX_LOCAL_LLM_ONLY`, `OMNUX_DISABLE_CLOUD_LLM` 요청 여부와 로컬 모델 준비 상태를 읽기 전용으로 평가한다.
- `offlineMode.cloudProviderKeysPresent`는 클라우드 key env var 이름만 반환하고 값은 반환하지 않는다.
