...
public sealed class ProviderRateLimitHeaderParserTests
{
    [Fact]
    public void ParseExtractsAllRateLimitHeadersAndStampsCapturedAt()
    {
        ...
        var snapshot = ProviderRateLimitHeaderParser.Parse(response.Headers, capturedAt);
    ...
    public void ParseReturnsNullableFieldsWhenHeadersAreAbsent()
    {
        ...
        var snapshot = ProviderRateLimitHeaderParser.Parse(response.Headers, DateTimeOffset.UtcNow);
    ...
    public void ParseStoresRetryAfterCooldownOnlyForRateLimitedResponses()
    {
        ...
        var regular = ProviderRateLimitHeaderParser.Parse(response.Headers, capturedAt);
        var limited = ProviderRateLimitHeaderParser.Parse(response.Headers, capturedAt, isRateLimited: true);
    ...
    public void ParseClampsLargeRetryAfterCooldown()
    {
        ...
        var snapshot = ProviderRateLimitHeaderParser.Parse(response.Headers, capturedAt, isRateLimited: true);
