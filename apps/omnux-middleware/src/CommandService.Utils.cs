...
public sealed partial class CommandService
{
    ...
    )
    {
        try
        {
            ...
            if (provider != "none" && !string.IsNullOrWhiteSpace(selectedAssistant))
            {
                ...
                var prompt = $"""
                            Make a one-sentence title for the conversation below, in the same language as the messages.
                            조건:
                            - 최대 28자
                            - 불필요한 따옴표/머리말 금지
                            - 제목만 출력
    ...
    )
    {
        ...
        var summaryPrompt = $"""
                            ...
                            Keep only the context needed to continue, compressed in the same language as the conversation.
                            반드시 포함:
                            1) 사용자 목표
                            2) 결정된 기술 선택/제약
                            3) 아직 남은 작업
