...
public sealed partial class WebSocketGateway
{
    ...
    public WebSocketGateway(
        ...
        ICodingProjectChangeService projectChanges,
    ...
    )
    {
        ...
        _projectCommandDispatcher = new WsProjectCommandDispatcher(projectService, projectChanges);
    ...
    private string BuildDashboardExternalUrlsJson(bool externalDashboardEnabled)
    {
        var urls = externalDashboardEnabled
            ? GetPrivateNetworkAddresses()
                .Select(address => BuildDesktopUiExternalUrl(address, _port))
    ...
    // 외부접속 주소는 미들웨어 자신의 HTTP 포트(_port, 기본 41880)를 가리켜야 한다.
    // 이 포트는 외부접속 활성 시 0.0.0.0 으로 바인딩되며 desktop/dist 를 그대로 서빙하므로
    // 개발/패키징 빌드 모두에서 LAN 기기가 접속할 수 있다. (Vite UI 포트 1420 은 패키징 빌드에
    // 존재하지 않고 개발 모드에서도 LAN 바인딩이 보장되지 않아 외부접속에 부적합하다.)
    internal static string BuildDesktopUiExternalUrl(string address, int port)
    {
    ...
    private Task SendConversationsAsync(WebSocket socket, SemaphoreSlim sendLock, string scope, string mode, CancellationToken cancellationToken)
        => SendConversationsAsync(socket, sendLock, scope, mode, cancellationToken, null);
    ...
    )
    {
        await SendTextAsync(
            ...
            + $"\"requestId\":\"{EscapeJson(requestId ?? string.Empty)}\","
    ...
    internal static ClientMessage? ParseClientMessage(string json)
    {
        try
        {
            ...
            if (doc.RootElement.TryGetProperty("url", out var webFetchUrlElement)
                || doc.RootElement.TryGetProperty("webFetchUrl", out webFetchUrlElement))
            {
            ...
            if (doc.RootElement.TryGetProperty("grokModel", out var grokModelElement))
            {
                if (grokModelElement.ValueKind is not (JsonValueKind.String or JsonValueKind.Null)) return null;
                grokModel = grokModelElement.GetString() ?? "none";
            }
            ...
            if (doc.RootElement.TryGetProperty("task", out var taskElement)
                || doc.RootElement.TryGetProperty("spawnTask", out taskElement))
            {
            ...
            if (doc.RootElement.TryGetProperty("ts", out var timestampElement)
                || doc.RootElement.TryGetProperty("timestamp", out timestampElement))
            {
