...
export function ChatHistory({ canRequest, open, onToggle }: { canRequest: boolean; open: boolean; onToggle: (open: boolean) => void }) {
  ...
  return <details className="chat-fold chat-history" open={open} onToggle={event => { onToggle(event.currentTarget.open); if (!event.currentTarget.open && state.sidePanel === "info") state.setSidePanel(null); }}>
    <summary>대화 기록</summary><div className="chat-fields">
