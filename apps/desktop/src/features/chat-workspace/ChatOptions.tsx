...
function ModelField({ provider, worker = false }: { provider: AskModelProvider; worker?: boolean }) {
  ...
  return <label>{PROVIDER_LABEL[provider]} {worker ? "워커 모델" : "응답 모델"}
...
export function ChatOptions({ canRequest }: { canRequest: boolean }) {
  ...
  return <div className="chat-fields">
      <div className="chat-columns">
        <label>모드<select value={state.chatMode} disabled={state.pending} onChange={event => { if (event.target.value !== state.chatMode) state.setChatMode(event.target.value as AskChatMode); }}><option value="single">싱글</option><option value="orchestration">오케스트레이션</option><option value="multi">멀티</option></select></label>
      ...
      {state.chatMode !== "single" && <fieldset><legend>워커 모델</legend><div className="chat-columns">{PROVIDER_KEYS.map(provider => <ModelField key={provider} provider={provider} worker />)}</div></fieldset>}
      <div className="chat-checks">
        ...
        <label><input type="checkbox" checked={state.thinkPlus} disabled={state.pending} onChange={event => state.setThinkPlus(event.target.checked)} />Think+</label>
        {isSpeechSupported() && <label><input type="checkbox" checked={speech.autoSpeak} onChange={speech.toggleAutoSpeak} />답변 자동 읽기</label>}
      </div>
      <details className="chat-fold"><summary>제공자마다 기본 모델</summary><div className="chat-columns">{PROVIDER_KEYS.map(provider => <ModelField key={provider} provider={provider} />)}</div></details>
