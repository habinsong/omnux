...
// 모델/키 그룹의 세부 항목들이 활성화될 때 한 번 로드한다.
export function useLlmSettingsLoad(canRequest: boolean) {
  const loadLlmServices = useSettingsStore((state) => state.loadLlmServices);
  const loadCredentials = useProviderCredentialsStore((state) => state.loadSettings);
  useEffect(() => {
    if (canRequest) {
      loadLlmServices();
      loadCredentials();
...
// ── 세부 항목: 연동 키 ──────────────────────────────────────────
export function LlmKeysCard({ canRequest, onError }: { canRequest: boolean; onError: CardErrorHandler }) {
...
// ── 세부 항목: CLI 인증 ─────────────────────────────────────────
export function CliAuthCard({ store, canRequest, onError }: { store: Store; canRequest: boolean; onError: CardErrorHandler }) {
  return (
    <CardBoundary title="CLI 인증 상태" card="runtime" onError={onError}>
      <div className="grid grid-cols-1 gap-2 xl:grid-cols-2">
        <CliStatusRow title="GitHub Copilot CLI" detail={store.copilotStatus.detail} status={store.copilotStatus.text}>
          <Button variant="outline" size="sm" onClick={store.refreshCliStatus} disabled={!canRequest}>
...
// ── 세부 항목: 사용량 ───────────────────────────────────────────
export function LlmUsageCard({ store, onError }: { store: Store; onError: CardErrorHandler }) {
  return (
