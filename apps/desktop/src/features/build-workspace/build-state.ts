...
export const useBuildWorkspace = create<State>((set, get) => ({
  ...
  run: () => {
    ...
    const fields = { text: input, scope: "coding", mode: settings.mode, conversationId: state.activeId || undefined, conversationTitle: settings.title || undefined, project: settings.project || undefined, projectKey: settings.projectKey || undefined,
      ...
      webUrls: Array.from(new Set(input.match(/https?:\/\/[^\s<>"']+/g) || [])), webSearchEnabled: settings.webSearch, thinkPlus: settings.think, reasoningEffort: settings.reasoning === "auto" ? undefined : settings.reasoning, contextBudget: settings.context, attachments: state.attachments, memoryNotes: settings.memory, skillScope: skillName.length ? skillScope : undefined, skillName: skillName.join(":") || undefined,
