...
export function BuildResultPanel({ connected }: { connected: boolean }) {
  ...
  return <section className="build-sheet" aria-label="빌드 결과">
    ...
    <div className="build-result-body">
      ...
      {result.workers.length > 0 && <details className="build-fold"><summary>모델별 결과 비교</summary><div className="build-form-fields"><label>살펴볼 결과<select value={state.target} disabled={Boolean(state.pending.run || state.pending.execute)} onChange={event => state.chooseTarget(event.target.value)}><option value="main">대표 결과 · {result.model}</option>{result.workers.map((worker, index) => <option key={index} value={`worker-${index}`}>{worker.model} · {statusName(worker.execution.status)}</option>)}</select></label></div></details>}
