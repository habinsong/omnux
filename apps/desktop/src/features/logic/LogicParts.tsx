...
import { ChevronUp, FolderOpen, FileText, Plus, RefreshCcw, Trash2, X } from "lucide-react";
...
import { type ContextPickerSelection } from "../context-picker/context-picker-store";
...
export function applyContextToLogicSelection(items: ContextPickerSelection[]) {
...
export function clampSize(node: EditableNode) {
  return {
...
export function outputPortPosition(node: EditableNode, port: string) {
...
export function inputPortPosition(node: EditableNode) {
  const size = clampSize(node);
  return { x: node.position.x, y: node.position.y + size.height / 2 };
}

export function bezierPath(sx: number, sy: number, tx: number, ty: number) {
...
export function bestPathField(node: EditableNode | null): string {
  if (!node) return "";
  const def = getLogicNodeDef(node.type);
...
export function LogicPathBrowserPanel({
  snapshot,
  selectedNode,
  canRequest
}: {
...
export function RunIoDetailPanel({ snapshot, selectedNodeId }: { snapshot: LogicRunSnapshot | null; selectedNodeId: string }) {
...
export function NodeInspector({ node }: { node: EditableNode }) {
  const store = useLogicStore();
  const def = getLogicNodeDef(node.type);
...
export function EdgeInspector({ edge, graph }: { edge: EditableEdge; graph: EditableGraph }) {
  const store = useLogicStore();
...
export function GraphSettings({ graph }: { graph: EditableGraph }) {
  const store = useLogicStore();
  return (
    <div className="space-y-2">
      <label className="block">
