...
omnux는 작게 나눈 구성요소를 WebSocket과 파일 상태 저장소로 묶는다. 데스크톱 앱과 텔레그램 봇은 서로 다른 제품처럼 동작하지 않고 같은 명령 계층을 통과한다.
...
각 핸들러는 CommandService private state가 아니라 자기 도메인 ApplicationService에만 의존한다.
...
| 기타 | Telemetry, GitAutomation, SessionReplay, SemanticSearch, Mcp, Terminal, Rag, LocalLlm, ClipboardVision 등 |

### WebSocket Dispatcher

`Ws*CommandDispatcher`(30개)가 도메인별 WebSocket 명령을 처리한다. 데스크톱 앱의 모든 요청이 이 계층을 통과한다.

### 정책(Policy) 클래스

`CommandService`와 `LlmRouter`는 진입점 역할만 맡고, 판정·파싱·프롬프트 조립은 단위 테스트 가능한 정책 클래스로 분리한다.
...
- 기타: `RemoteLimitedMessagePolicy`, `UniversalCodeExecutionSafetyPolicy`, `AdaptiveContextCompressionPolicy`, `PromptCachePolicy`, `RagRetrievalPreflightPolicy`, `MemoryTierPolicy`

정책은 `apps/omnux-middleware-tests`에 단위 테스트가 있고, `scripts/check-security-boundaries.mjs`가 계약을 검증한다.
...
2. 서버 메시지는 `desktop-message-gateway`를 통해 화면 store로 배포된다.
3. 화면은 UI 입력만 gateway에 넘기고, 필드명 변환과 허용 요청 목록은 gateway가 맡는다. 비즈니스 로직은 없다.

디자인 원칙:

- Tailwind CSS v4 시맨틱 토큰, 3가지 테마: Light(기본), Glass, Dark
...
- 시크릿은 환경변수, `*_FILE`, secure store(`~/.config/omnux/secrets.json`, 0600), macOS Keychain 경로로 분리한다.
- 외부접속 클라이언트는 OTP 요청 없이 제한 모드로 들어오지만, WebSocket 메시지 allowlist를 한 번 더 통과해야 한다.
...
- Markdown 렌더링은 raw HTML을 비활성화한다.
- Safe Refactor는 apply 직전에 파일 상태를 다시 확인한다. 코딩 실행은 workspace rollback을 지원한다.
- JSON 상태 파일 쓰기는 파일별 `.lock` lease를 잡고 원자 교체한다. 기존 정상 파일은 `.bak`로 남긴다.
- 코딩 실행은 실행별 폴더를 따로 만들고, `OMNUX_ENABLE_DYNAMIC_CODE=true`일 때만 로컬 코드 실행을 허용한다. 셸은 zsh → bash → sh 순으로 있는 것을 쓴다.
- Python sandbox는 로컬 신뢰 코드 실행용 제한기이며 OS 수준 보안 샌드박스가 아니다.
