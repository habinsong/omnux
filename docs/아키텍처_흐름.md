...
구성요소는 WebSocket으로 통신하고 상태를 파일에 저장한다. 데스크톱 앱과 텔레그램 봇은 같은 명령 계층을 통과한다.
...
- 코딩 실행은 실행마다 폴더를 따로 만든다. 로컬 코드 실행은 `OMNUX_ENABLE_DYNAMIC_CODE=true`일 때만 허용한다. 셸은 zsh → bash → sh 순으로 있는 것을 쓴다.
