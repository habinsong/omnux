...
스킬은 데스크톱 앱과 텔레그램 봇에서 같은 방식으로 동작한다. 대화 입력창의 스킬 배지나 슬래시 명령으로 활성화/해제할 수 있다.
...
- 질문/빌드 입력창의 스킬 배지를 끄면 UI 선택뿐 아니라 서버에 저장된 해당 대화의 sticky 스킬도 함께 해제된다.
- 질문 화면과 텔레그램 봇은 같은 스킬 감지/활성화/해제 흐름을 사용한다.
...
`/skill create`는 같은 이름의 기존 스킬을 조용히 덮어쓰지 않는다. 기존 스킬을 고치려면 데스크톱 도구 화면에서 해당 스킬을 열어 저장한다.
