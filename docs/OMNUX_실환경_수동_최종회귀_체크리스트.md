...
릴리스 직전에 사람이 직접 눌러 보는 체크리스트다. 자동 테스트가 지나도 실제 데스크톱 앱과 텔레그램에서 깨지는 지점은 여기서 잡는다.
...
- [ ] 홈: 입력창, 바로가기, 이어서 작업하기 패널
- [ ] 질문: 싱글/오케스트레이션/멀티 전환, 마크다운 렌더링
- [ ] 빌드: 실행 폴더 생성, 최근 결과 복원
- [ ] 자동화: 루틴 생성/즉시 실행
- [ ] 탐색: 웹 검색, URL 가져오기, 브라우저
- [ ] 규칙: 로직 그래프 저장/실행
- [ ] 작업/노트: 계획 생성/리뷰/승인, 노트 저장
- [ ] 도구: 스킬 목록과 활성화/중지
- [ ] 리뷰: Safe Refactor preview 생성
- [ ] 설정: 모델·키 탭, provider 상태
- [ ] 테마 전환: Light/Glass/Dark
- [ ] Command Palette (⌘K / Ctrl+K)
- [ ] 좁은 폭에서 메뉴 버튼으로 내비 열기/닫기

## 백업

- [ ] 설정 > 메모리에서 portable package 설명, `portable-package-only` 동기화 모드, 충돌 정책 표시
...
- [ ] 외부 원격 머신에서 `node scripts/gist-bridge-remote-qa.mjs --token <GITHUB_TOKEN>` 실행 후 `outboundUploadOk` / `inboundDownloadOk`가 모두 `true`인지 확인
